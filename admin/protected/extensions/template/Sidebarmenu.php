...
class Sidebarmenu extends CWidget
{
     ...
     */
    protected function renderMenuRecursive($items,$isChild=false)
    {
        $count=0;
        $n=count($items);
        foreach($items as $item)
        {
            ...
            $isActive = $item['active'] && $this->activeCssClass!='' ? true : false;
            $hasCild = isset($item['items']) && count($item['items']) ? true : false;
            $menu=$this->renderMenuItem($item,$isChild,$hasCild,$isActive);
     ...
     * @since 1.1.6
     */  
    protected function renderMenuItem($item,$isChild,$hasCild,$isActive)
    {
        ...
        if(!isset($item['url']))
            $item['url'] = '#';
        return CHtml::link($label.$numberTag,$item['url'],isset($item['linkOptions']) ? $item['linkOptions'] : array());
    }
